...
# Abrir a página da playlist no navegador
driver.get(url_playlist)

# Esperar a página carregar completamente
try:
    # Espera até que os vídeos da playlist sejam carregados (ajuste de acordo com a estrutura da página)
    WebDriverWait(driver, 20).until(
        EC.presence_of_all_elements_located((By.XPATH, "//a[@class='yt-simple-endpoint style-scope yt-formatted-string']"))
    )
                    ...
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        try:
                            ...
                            if 'channel_id' in info_dict:
                                ...
                                m3u_file.write(f"#EXTINF:-1 group-title=\"Reality Show's Live\" tvg-logo=\"{thumbnail_url}\" tvg-id=\"{channel_id}\" ,{video_title}\n")
